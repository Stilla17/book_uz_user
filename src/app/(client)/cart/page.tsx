...
export default function CartPage() {
  ...
  const floatingIcons = [ShoppingCart, BookOpen, Headphones, Sparkles, Crown, Zap, Award, Gem, Flower2, Sun, Moon, Cloud, Coffee, Compass, TrendingUp];
  ...
  const loadCart = async () => {
    try {
      ...
      if (response.data?.success && response.data.data) {
        ...
        const totalItems = response.data.data.items.reduce((sum: number, item: CartItem) => sum + item.quantity, 0);
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-gray-50 to-white dark:from-slate-900 dark:to-slate-800 relative overflow-hidden">
        ...
        <div className="text-center relative z-10">
          <div className="relative">
            ...
            <ShoppingCart className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-[#005CB9] dark:text-blue-400" size={24} />
          </div>
          <p className="text-gray-500 dark:text-gray-400 animate-pulse">Savat yuklanmoqda...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="container mx-auto px-4 max-w-7xl relative z-10">
        ...
        {process.env.NODE_ENV === 'development' && (
          <motion.div 
            ...
            className="mb-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <motion.div
                ...
                className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-xl shadow-lg border border-gray-100 dark:border-slate-700 p-4"
              >
              ...
              {cart.items.map((item, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-xl shadow-lg border border-gray-100 dark:border-slate-700 p-4 hover:shadow-xl transition-all"
                  >
                    <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      {/* Product Image */}
                      <Link href={`/book/${item.product.slug || item.product._id}`} className="flex-shrink-0">
                        <motion.div 
                          whileHover={{ scale: 1.05 }}
                          className="relative w-24 h-32 rounded-lg overflow-hidden shadow-md"
                        >
                          <Image
                            src={item.product.images?.[0] || "https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?q=80&w=1887"}
                            ...
                            onError={(e) => {
                              ...
                              target.src = "https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?q=80&w=1887";
                            }}
                          />
                        </motion.div>
                      </Link>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                          <div>
                            <Link 
                              ...
                              className="text-lg font-bold text-gray-900 dark:text-white hover:text-[#005CB9] dark:hover:text-blue-400 transition-colors"
                            >
                              {getProductTitle(item.product)}
                            </Link>
                            
                            {item.product.author?.name && (
                              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{item.product.author.name}</p>
                            )}
                            ...
                            <div className="flex items-center gap-2 mt-2">
                              ...
                              {/* Format Badge */}
                              {item.product.format && (
                                <div className="flex items-center gap-1 px-2 py-0.5 bg-gray-100 dark:bg-slate-700 rounded-full">
                                  {getFormatIcon(item.product.format)}
                                  <span className="text-[10px] font-medium text-gray-600 dark:text-gray-400">
                                    {item.product.format === 'audio' ? 'Audio' : 
                                     item.product.format === 'ebook' ? 'E-kitob' : 'Qog\'oz'}
                                  </span>
                                </div>
                              )}
                            </div>
                          ...
                          <div className="flex flex-col items-end gap-4">
                            ...
                            <div className="flex items-center gap-2">
                              <motion.button
                                ...
                                className="p-1.5 rounded-lg border border-gray-200 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed text-gray-700 dark:text-gray-300"
                              >
                                <Minus size={14} />
                              </motion.button>
                              ...
                              >
                                {updating === item.product._id ? (
                                  <Loader2 size={14} className="animate-spin mx-auto text-blue-500 dark:text-blue-400" />
                                ) : (
                                  item.quantity
                                )}
                              </motion.span>
                            ...
                            <div className="text-right">
                              <p className="text-sm text-gray-400 dark:text-gray-500">Umumiy:</p>
                              <motion.p 
            ...
            <div className="lg:col-span-1">
              <motion.div
                ...
                className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-xl shadow-lg border border-gray-100 dark:border-slate-700 p-6 sticky top-24"
              >
                <h2 className="text-lg font-black text-gray-900 dark:text-white mb-4">Buyurtma haqida</h2>
                  ...
                  {appliedPromo ? (
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        className="flex-1 bg-white dark:bg-slate-700 border-gray-200 dark:border-slate-600 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500"
                      />
                ...
                >
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500 dark:text-gray-400">Tanlangan mahsulotlar ({getSelectedCount()} ta)</span>
                    <motion.span 
                ...
                >
                  <Button
                    ...
                    className="w-full bg-gradient-to-r from-[#005CB9] to-[#FF8A00] dark:from-blue-600 dark:to-orange-600 text-white font-bold py-6 text-lg rounded-xl hover:shadow-lg dark:hover:shadow-2xl dark:hover:shadow-blue-900/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed mb-4"
                  >
