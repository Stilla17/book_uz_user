    ...
    return (
        <div>
            ...
            <DropdownMenu>
                <DropdownMenuTrigger
                    disabled={disabled}
                    className='flex h-12 w-full items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-left text-sm text-slate-700 shadow-sm outline-none transition-colors hover:bg-white disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:hover:bg-slate-900'>
                    <span className={selectedLabels.length ? 'truncate' : 'truncate text-slate-400'}>{triggerText}</span>
                ...
                <DropdownMenuContent className='max-h-80 w-[var(--radix-dropdown-menu-trigger-width)] overflow-y-auto rounded-2xl p-1'>
                    ...
                    {isSearching && <div className='px-3 py-3 text-sm text-slate-400'>Qidirilmoqda...</div>}
                    ...
                    {!isSearching && visibleGroups.length === 0 && visibleOptions.length === 0 && (
                        <div className='px-3 py-5 text-center text-sm text-slate-400'>Topilmadi</div>
                    )}
                </DropdownMenuContent>
            </DropdownMenu>
        </div>
